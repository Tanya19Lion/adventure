...
gulp.task("scss", function () {
	...
	return gulp
		...
		.pipe(gulp.dest("app/css")) //кладём итоговый файл в директорию build/css
		.pipe(
			browserSync.reload({
				stream: true,
			}),
...
gulp.task("style", function () {
	...
	return gulp
		...
		.pipe(gulp.dest("app/css")) //кидаем готовый файл в директорию
		.pipe(size());
});
